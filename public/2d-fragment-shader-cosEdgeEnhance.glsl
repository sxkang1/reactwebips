...
void main() {
  ...
  vec3 colorSum;
  float nGrayThreshold = 52000.0/65535.0/255.0;
  float nMinMaxThreshold = 4000.0/65535.0/255.0;
  float fExpIndexW = 3.0;
  float fExpIndexB = 4.0;
  float fEnhanceDegreeW = 4.0;
  float fEnhanceDegreeB = 8.0;
  float fDifThreshold = 0.0005;
  ...
  if(rgb2hsl((texture(u_image, v_texCoord + onePixel * vec2(0,0))).rgb).z<1.0){
      ...
      for(int i =0; i<9;i++){
        ...
        float dif;
        if(tempL > nGrayThreshold){
            dif = tempL - mediumL;
            dif = min(dif,1.0);
            tempL = tempL + (tempL-mediumL)*3.0;
        }
        else{
            dif = tempL - mediumL;
            if(abs(dif*tempL*tempL*tempL)<fDifThreshold){
                dif *= 2.0;
                dif = min(dif,1.0);
                if(dif>=0.0){
                    tempL = tempL+expIndex*cos(3.14/2.0*dif)*0.001+((tempL-mediumL)*fEnhanceDegreeW);//参数5，亮边缘增强程度
                }
                else{
                    tempL = tempL-expIndex*cos(3.14/2.0*dif)*0.001+((tempL-mediumL)*fEnhanceDegreeB);//参数6，暗边缘增强程度
                }
            }
            else{
                dif *= 2.0;
                dif = min(dif,1.0);
                if(dif>=0.0){
                    tempL = tempL + expIndex*cos(3.14/2.0*dif)*0.01+((tempL-mediumL)*fEnhanceDegreeW);
                }
                else{
                    tempL = tempL - expIndex*cos(3.14/2.0*dif)*0.01+((tempL-mediumL)*fEnhanceDegreeB);
                }
            }
        }
