...
export default class WebGl extends Component {
    ...
    //         ctx.drawImage(img, 0, 0, this.width, this.height, this.imgX, this.imgY, img.width * this.imgScale, img.height * this.imgScale); //参数可自定义
    ...
    MouseWheel = (event) => {
        ...
        // console.log('event.deltaY----',event.deltaY)
         console.log('Detector.scaleMatrix[0]----',Detector.scaleMatrix[0])
        // console.log('-1 / 4---',-1 / 4)
        ...
        //     Detector.translationMatrix[0] = this.imgX * 2 - pos.x;
        //     Detector.translationMatrix[1] = this.imgY * 2 - pos.y;
        // } else {
        //     // this.imgScale /= 2;
        //     // this.imgX = this.imgX * 0.5 +  pos.x * 0.5;
        //     // this.imgY = this.imgY * 0.5 +  pos.y * 0.5;

        //     Detector.translationMatrix[0] = this.imgX * 0.5 +  pos.x * 0.5;
        //     Detector.translationMatrix[1] = this.imgY * 0.5 +  pos.y * 0.5;
        // }

        if (event.deltaY < 0) {
            Detector.scaleMatrix[0] += 0.1;
            Detector.scaleMatrix[1] += 0.1;
            // Detector.translationMatrix[0] = Detector.translationMatrix[0] * 2 - pos.x;
            // Detector.translationMatrix[1] = Detector.translationMatrix[1] * 2 - pos.y;
            // Detector.scaleMatrix[0] *= event.deltaY * -2;
            // Detector.scaleMatrix[1] *= event.deltaY * -2;
        }
        else {
            if (Detector.scaleMatrix[0] >= 0.1) {
                Detector.scaleMatrix[0] -= 0.1;
                Detector.scaleMatrix[1] -= 0.1;
                // Detector.translationMatrix[0] = Detector.scaleMatrix[0] * 0.5 +  pos.x * 0.5;
                // Detector.translationMatrix[1] = Detector.scaleMatrix[1] * 0.5 +  pos.y * 0.5;
            }
        ...
        // 以图像中心为中心进行缩放;
        // console.log('Detector.scaleMatrix[0]--',Detector.scaleMatrix[0])
        // console.log('originalScaleMatrix[0]--',originalScaleMatrix[0])
...
//         let width = this.image.width * Detector.scaleMatrix[0];
//         let height = this.image.height * Detector.scaleMatrix[1];

//         let sx = this.refs.canvas.width / 2 - width / 2;
//         let sy = this.refs.canvas.height / 2 - height / 2 ;

//  Detector.translationMatrix = [sx-width, sy-height];
        Detector.translationMatrix = [Detector.translationMatrix[0] - width, Detector.translationMatrix[1] - height / 2];
        // Detector.translationMatrix = [this.imgX, this.imgY];
        // console.log( Detector.translationMatrix)
        // drawImage();
        this.DrawEffection();
