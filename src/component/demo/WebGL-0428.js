...
export default class WebGl extends Component {
    ...
    handleBtnAlgorithmServices = (operaionType) => {//例如："histogram" 
        ...
        //1、调用后端接口 	GetOperationsJPG  接口中 imagePath即 this.imagesPath
        console.log(this.AlgorithmServicesList[operaionType])
        if (this.AlgorithmServicesList[operaionType]) {
            // alert(1)
            let params =  {  
                'imagePath': this.imagesPath,
                'operators': {
                    'histogram':this.AlgorithmServicesList[operaionType]
                }
            }
            console.log(params)
            // http://127.0.0.1:8000    http://10.44.161.217:8000
            axios.post('http://127.0.0.1:8000/WebIPS/GetOperationsJPG',
                params
            ).then((res) => {
            ...
            let str = this.imagesPath;
            let index = str.lastIndexOf("-")
            str = str.substring(0,index);
            
            this.imagesPath = str + '.jpg'
            this.DrawScene(this.shaderVarray);
            this.setState({histogramLoading: false})
        }
        //2、接口返回 200  执行 this.DrawScene(this.shaderVarray);
        // this.imagesPath = '张建返回的图像路径';
    }
    ...
    MouseMove = (event) => {
        ...
        // console.log('this.lMouseDownX--',this.lMouseDownX)
        // console.log('this.lMouseDownY--',this.lMouseDownY)
    }

    windowToCanvas(x, y) {
        var box = this.refs.canvas.getBoundingClientRect();  //这个方法返回一个矩形对象，包含四个属性：left、top、right和bottom。分别表示元素各边与页面上边和左边的距离
        return {
    ...
    MouseWheel = (event) => {
        ...
        let windowsTocanvas = this.windowToCanvas(event.clientX, event.clientY)
        console.log("x--",windowsTocanvas.x)
        console.log("y--",windowsTocanvas.y)
        ...
        //以图像中心为中心进行缩放;
        // var width = this.image.width * Detector.scaleMatrix[0];
        // var height = this.image.height * Detector.scaleMatrix[1];
        // if(this.lMouseDownX != -1 && this.lMouseDownY != -1){
        //     var xBegin = ((this.refs.canvas.width - width)/2)*originalScaleMatrix[0];
        //     var yBegin = ((this.refs.canvas.height -height) / 2)*originalScaleMatrix[1];
        ...
        let imageWidth = this.image.width*Detector.scaleMatrix[0];
        let imageHeight = this.image.height*Detector.scaleMatrix[1];
       
        console.log("imagewidth--",imageWidth)
        console.log("imageHeight--",imageHeight)
       
        var imageX = this.refs.canvas.width/2 - imageWidth/2;
        var imageY = this.refs.canvas.height/2 - imageHeight/2;

        Detector.translationMatrix = [imageX, imageY];
       console.log('Detector.translationMatrix---',Detector.translationMatrix)
                    ...
                    <Col span={10} className="">
                        ...
                        <Row style={{ marginTop: '20px' }}>
                            <Col span={24}>
                                ...
                                <Button className="btn btn-Histogram" title="Histogram" loading={this.state.histogramLoading} type={this.state.operationList[12].btnState}  onClick={this.handleBtnAlgorithmServices.bind(this, 'Histogram')}></Button>                                
                                <Button className="btn btn-Revert" title="Revert" style={{ margin: '0 20px' }} type={this.state.operationList[3].btnState}  onClick={this.handleBtnClick.bind(this, 'Revert')}></Button>
                            </Col>
                        </Row>
                    </Col>
                </Row>
