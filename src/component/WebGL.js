...
export default class WebGl extends Component {
  constructor(props) {
    ...
    this.kernels = {
      ...
      Smooth1: [0.045, 0.122, 0.045, 0.122, 0.332, 0.122, 0.045, 0.122, 0.045],
      Smooth2: [1, 2, 1, 2, 4, 2, 1, 2, 1],
      Smooth3: [0, 1, 0, 1, 1, 1, 0, 1, 0],
      ...
      EdgeEnhance1: [-1, -1, -1, -1, 11, -1, -1, -1, -1],
      EdgeEnhance2: [-1, -1, -1, -1, 10, -1, -1, -1, -1],
      EdgeEnhance3: [-1, -1, -1, -1, 9, -1, -1, -1, -1],
  ...
  handleBtnAlgorithmServices = operaionType => {
    //例如："histogram"
    this.btnStyle(operaionType);
    this.setState({ histogramLoading: true });
    console.log("this.operaionType---", operaionType);
    this.AlgorithmServicesList[operaionType] = !this.AlgorithmServicesList[
      operaionType
    ];
    //    console.log(this.AlgorithmServicesList[operaionType])
    console.log(
      this.state.operationList[12].btnState == "primary" ? true : false
    );
    let params = {
      ...
      operators: {
        ...
        PseudoColorize: {
          IsEnabled: this.flag,
          Options:
            operaionType == "histogram" && this.flag == true
              ? "gray"
              : operaionType
        }
      }
    ...
    axios
      .post("http://10.44.161.217:8000/WebIPS/GetOperationsJPG", params)
      .then(res => {
  ...
  getImagePath = (str) => {
    ...
    axios
      .post("http://10.44.161.217:5050/WebIPS/GetImage",params,
      {
        headers: {
      ...
      .then(res => {
        console.log(res.data.resultData[0].jpg_path)
      //  this.state.Base64Url = "data:image/jpeg;base64," + res.data.resultData.jpg_path
      //  this.setState({
      //    imageflag: true
      //  })
      })
  ...
  submitInfo = () => {
    let params = {
      picno: this.props.location.state.path,
    ...
    axios.post("http://10.44.161.217:5050/WebIPS/Submit",params,
    {
      headers: {
      "Content-Type": "application/json; charset=utf-8"
    }
  ...
  componentDidMount() {
   ...
   // }
    ...
    if (this.props.location.state) {
      //判断当前有参数
      //this.getImagePath(this.props.location.state.path);
      //if (this.state.imageflag == true) {
        ...
        axios.post("http://10.44.161.217:5050/WebIPS/GetImage",params,
        {
          headers: {
          "Content-Type": "application/json; charset=utf-8"
        }
        ...
        .then(res => {
          console.log(res.data.resultData[0].jpg_path)
//        this.state.Base64Url = "data:image/jpeg;base64," + res.data.resultData[0].jpg_path
          this.state.Base64Url = res.data.resultData[0].jpg_path
        console.log("1111111111",this.state.Base64Url)
        console.log("222222222")
        recvParam = res.data.resultData[0].jpg_path;
  ...
  InitWebGLProgram = () => {
    ...
    this.kernelProgram.program = WebglUtils.initProgram(
      this.webgl,
      vsSource,
      fsSource
    );
    this.kernelProgram.a_Position = this.webgl.getAttribLocation(
      this.kernelProgram.program,
      "a_position"
    ); //返回顶点着色器中顶点变量的索引地址
    this.kernelProgram.a_TexCoord = this.webgl.getAttribLocation(
      this.kernelProgram.program,
      "a_texCoord"
    );
    this.kernelProgram.imageLocation = this.webgl.getUniformLocation(
      this.kernelProgram.program,
      "u_image"
    );
    this.kernelProgram.resolutionLocation = this.webgl.getUniformLocation(
      this.kernelProgram.program,
      "u_resolution"
    );
    this.kernelProgram.translationLocation = this.webgl.getUniformLocation(
      this.kernelProgram.program,
      "u_translation"
    );
    this.kernelProgram.scaleLocation = this.webgl.getUniformLocation(
      this.kernelProgram.program,
      "u_scale"
    );
    this.kernelProgram.kernelLocation = this.webgl.getUniformLocation(
      this.kernelProgram.program,
      "u_kernel[0]"
    );
    this.kernelProgram.kernelWeightLocation = this.webgl.getUniformLocation(
      this.kernelProgram.program,
      "u_kernelWeight"
    );
    ...
    this.reverseProgram.program = WebglUtils.initProgram(
      this.webgl,
      vsSource,
      fsSource
    );
    this.reverseProgram.a_Position = this.webgl.getAttribLocation(
      this.reverseProgram.program,
      "a_Position"
    ); //返回顶点着色器中顶点变量的索引地址
    this.reverseProgram.a_TexCoord = this.webgl.getAttribLocation(
      this.reverseProgram.program,
      "a_TexCoord"
    );
    this.reverseProgram.resolutionLocation = this.webgl.getUniformLocation(
      this.reverseProgram.program,
      "u_resolution"
    );
    this.reverseProgram.translationLocation = this.webgl.getUniformLocation(
      this.reverseProgram.program,
      "u_translation"
    );
    this.reverseProgram.scaleLocation = this.webgl.getUniformLocation(
      this.reverseProgram.program,
      "u_scale"
    );
    ...
    this.grayProgram.program = WebglUtils.initProgram(
      this.webgl,
      vsSource,
      fsSource
    );
    this.grayProgram.a_Position = this.webgl.getAttribLocation(
      this.grayProgram.program,
      "a_Position"
    ); //返回顶点着色器中顶点变量的索引地址
    this.grayProgram.a_TexCoord = this.webgl.getAttribLocation(
      this.grayProgram.program,
      "a_TexCoord"
    );
    this.grayProgram.resolutionLocation = this.webgl.getUniformLocation(
      this.grayProgram.program,
      "u_resolution"
    );
    this.grayProgram.translationLocation = this.webgl.getUniformLocation(
      this.grayProgram.program,
      "u_translation"
    );
    this.grayProgram.scaleLocation = this.webgl.getUniformLocation(
      this.grayProgram.program,
      "u_scale"
    );
    this.grayProgram.grayValue = this.webgl.getUniformLocation(
      this.grayProgram.program,
      "u_grayValue"
    );
    ...
    this.grayImageProgram.program = WebglUtils.initProgram(
      this.webgl,
      vsSource,
      fsSource
    );
    this.grayImageProgram.a_Position = this.webgl.getAttribLocation(
      this.grayImageProgram.program,
      "a_Position"
    ); //返回顶点着色器中顶点变量的索引地址
    this.grayImageProgram.a_TexCoord = this.webgl.getAttribLocation(
      this.grayImageProgram.program,
      "a_TexCoord"
    );
    this.grayImageProgram.resolutionLocation = this.webgl.getUniformLocation(
      this.grayImageProgram.program,
      "u_resolution"
    );
    this.grayImageProgram.translationLocation = this.webgl.getUniformLocation(
      this.grayImageProgram.program,
      "u_translation"
    );
    this.grayImageProgram.scaleLocation = this.webgl.getUniformLocation(
      this.grayImageProgram.program,
      "u_scale"
    );
    ...
    fsSource = this.shaderVarray.findShader(
      "2d-fragment-shader-LogEdgeEnhance"
    );
    this.logEdgeEnhanceProgram.program = WebglUtils.initProgram(
      this.webgl,
      vsSource,
      fsSource
    );
    this.logEdgeEnhanceProgram.a_Position = this.webgl.getAttribLocation(
      this.logEdgeEnhanceProgram.program,
      "a_position"
    ); //返回顶点着色器中顶点变量的索引地址
    this.logEdgeEnhanceProgram.a_TexCoord = this.webgl.getAttribLocation(
      this.logEdgeEnhanceProgram.program,
      "a_texCoord"
    );
    this.logEdgeEnhanceProgram.imageLocation = this.webgl.getUniformLocation(
      this.logEdgeEnhanceProgram.program,
      "u_image"
    );
    this.logEdgeEnhanceProgram.resolutionLocation = this.webgl.getUniformLocation(
      this.logEdgeEnhanceProgram.program,
      "u_resolution"
    );
    this.logEdgeEnhanceProgram.translationLocation = this.webgl.getUniformLocation(
      this.logEdgeEnhanceProgram.program,
      "u_translation"
    );
    this.logEdgeEnhanceProgram.scaleLocation = this.webgl.getUniformLocation(
      this.logEdgeEnhanceProgram.program,
      "u_scale"
    );
    ...
    vsSource = this.shaderVarray.findShader("2d-vertex-shader-kernel");
    fsSource = this.shaderVarray.findShader(
      "2d-fragment-shader-cosEdgeEnhance"
    );
    this.cosEdgeEnhanceProgram.program = WebglUtils.initProgram(
      this.webgl,
      vsSource,
      fsSource
    );
    this.cosEdgeEnhanceProgram.a_Position = this.webgl.getAttribLocation(
      this.cosEdgeEnhanceProgram.program,
      "a_position"
    ); //返回顶点着色器中顶点变量的索引地址
    this.cosEdgeEnhanceProgram.a_TexCoord = this.webgl.getAttribLocation(
      this.cosEdgeEnhanceProgram.program,
      "a_texCoord"
    );
    this.cosEdgeEnhanceProgram.imageLocation = this.webgl.getUniformLocation(
      this.cosEdgeEnhanceProgram.program,
      "u_image"
    );
    this.cosEdgeEnhanceProgram.resolutionLocation = this.webgl.getUniformLocation(
      this.cosEdgeEnhanceProgram.program,
      "u_resolution"
    );
    this.cosEdgeEnhanceProgram.translationLocation = this.webgl.getUniformLocation(
      this.cosEdgeEnhanceProgram.program,
      "u_translation"
    );
    this.cosEdgeEnhanceProgram.scaleLocation = this.webgl.getUniformLocation(
      this.cosEdgeEnhanceProgram.program,
      "u_scale"
    );
  };
  ...
  SetPositionLocation = (xBegin, yBegin, positionLocation) => {
    var data = new Float32Array([
      ...
      yBegin, //左上角——v0
      xBegin,
      yBegin + this.image.height, //左下角——v1
      xBegin + this.image.width,
      yBegin, //右上角——v2
      xBegin + this.image.width,
      yBegin + this.image.height //右下角——v3
    ]);
  ...
  DrawScene = shaderVarray => {
    ...
    image.src = this.state.Base64Url 
  ...
  renderMain = (image, shaderVarray) => {
    ...
    if (this.image.height > this.refs.canvas.height) {
      ...
      this.image.width = this.refs.canvas.width;
    }
    ...
    this.webgl.bufferData(
      this.webgl.ARRAY_BUFFER,
      Detector.textureData,
      this.webgl.STATIC_DRAW
    ); //STATIC_DRAW：静态绘制模式
    this.webgl.vertexAttribPointer(
      this.kernelProgram.a_TexCoord,
      2,
      this.webgl.FLOAT,
      false,
      0,
    ...
    this.kernelProgram.originalImageTexture = WebglUtils.createAndSetupTexture(
      this.webgl
    );
    this.webgl.texImage2D(
      this.webgl.TEXTURE_2D,
    ...
    // Detector.translationMatrix[1] = yBegin;
  ...
  DrawEffection = () => {
    ...
    this.webgl.bindTexture(
      this.webgl.TEXTURE_2D,
      this.kernelProgram.originalImageTexture
    );
    ...
    this.state.operationList.forEach(item => {
      if (item.using) {
        if (item.using === true && item.programCategory === "kernel") {
          // 使用两个帧缓冲中的一个
          WebglUtils.setFramebuffer(
            this.webgl,
            this.framebuffers[count % 2],
            this.kernelProgram,
            this.image.width,
            this.image.height
          );
          ...
          this.webgl.vertexAttribPointer(
            this.reverseProgram.a_TexCoord,
            2,
            this.webgl.FLOAT,
            false,
            0,
          ...
          this.webgl.uniform2fv(this.reverseProgram.translationLocation, [
            0,
            0
          ]);
          // Set the scale.
          ...
          this.webgl.uniform2fv(this.grayProgram.grayValue, [
            this.state.nGrayMin / 255.0,
            this.state.nGrayMax / 255.0
          ]);
          ...
          this.webgl.vertexAttribPointer(
            this.grayImageProgram.a_TexCoord,
            2,
            this.webgl.FLOAT,
            false,
            0,
          ...
          this.webgl.uniform2fv(this.grayImageProgram.translationLocation, [
            0,
            0
          ]);
          // Set the scale.
          ...
          this.webgl.vertexAttribPointer(
            this.logEdgeEnhanceProgram.a_TexCoord,
            2,
            this.webgl.FLOAT,
            false,
            0,
          ...
          this.webgl.enableVertexAttribArray(
            this.logEdgeEnhanceProgram.a_Position
          ); //允许GPU读取数据
          this.webgl.enableVertexAttribArray(
            this.logEdgeEnhanceProgram.a_TexCoord
          );
          ...
          this.webgl.uniform2fv(
            this.logEdgeEnhanceProgram.translationLocation,
            [0, 0]
          );
          // Set the scale.
          this.webgl.uniform2fv(this.logEdgeEnhanceProgram.scaleLocation, [
            1,
            1
          ]);
          ...
          this.webgl.vertexAttribPointer(
            this.cosEdgeEnhanceProgram.a_TexCoord,
            2,
            this.webgl.FLOAT,
            false,
            0,
          ...
          this.webgl.enableVertexAttribArray(
            this.cosEdgeEnhanceProgram.a_Position
          ); //允许GPU读取数据
          this.webgl.enableVertexAttribArray(
            this.cosEdgeEnhanceProgram.a_TexCoord
          );
          ...
          this.webgl.uniform2fv(
            this.cosEdgeEnhanceProgram.translationLocation,
            [0, 0]
          );
          // Set the scale.
          this.webgl.uniform2fv(this.cosEdgeEnhanceProgram.scaleLocation, [
            1,
            1
          ]);
    ...
    // var aa = (this.refs.canvas.width - this.image.width)/2;
    // var bb = (this.refs.canvas.height - this.image.height)/2;
    // var temp1 = this.windowTocanvas(aa,bb);
    // Detector.translationMatrix[0] = temp1.x;
    //Detector.translationMatrix[1] = temp1.y;

    //最终设置位移矩阵和缩放矩阵;
    this.webgl.uniform2fv(
      this.kernelProgram.translationLocation,
      Detector.translationMatrix
    ); //设置位移
    console.log("最终绘制----", Detector.translationMatrix);
    this.webgl.uniform2fv(
      this.kernelProgram.scaleLocation,
      Detector.scaleMatrix
    ); //设置缩放
    WebglUtils.setFramebuffer(
      this.webgl,
      null,
  ...
  drawWithKernel = name => {
    this.webgl.uniform1fv(
      this.kernelProgram.kernelLocation,
      this.kernels[name]
    );
    this.webgl.uniform1f(
      this.kernelProgram.kernelWeightLocation,
      this.computeKernelWeight(this.kernels[name])
    );
  ...
  MouseUp = event => {
    event.preventDefault();
    this.prevMouseDownX = this.lMouseDownX;
    this.prevMouseDownY = this.lMouseDownY;
  ...
  windowTocanvas = (x, y) => {
    ...
    return {
      x: x - this.refs.canvas.width / bbox.width,
      y: y - this.refs.canvas.height / bbox.height
    };
    // return {
    //     x: x - bbox.left - (bbox.width - this.refs.canvas.width),
    //     y: y - (bbox.height - this.refs.canvas.height)/2
    // };
  };
  ...
  MouseWheel = event => {
    ...
    var tempX =
      (this.refs.canvas.width - Detector.scaleMatrix[0] * this.image.width) / 2;
    var tempY =
      (this.refs.canvas.height - Detector.scaleMatrix[1] * this.image.height) /
      2;
    ...
    console.log("缩放------", Detector.translationMatrix);
    // Detector.translationMatrix[0] =  temp1.x;
    // Detector.translationMatrix[1] =  temp1.y;
  ...
  render() {
    return (
      <div className="divBox">
        {/* <LeftOutlined className="positiIcon"/> */}
        {/* <div className="icons-list">
                    <SyncOutlined/>
                </div> */}
        {/* <Row className="headerBox">
          <Col span={4} className="headerBoxOne" />
        ...
        <Row className="contents">
          <Col>
            <canvas
              id="webgl"
              ref="canvas"
              style={{ width: "100%", height: "calc(77vh)" }}
        ...
        <Row className="bootoms">
          <Col span={5}>
            <div className="ant-slider-div">
              ...
              >
                <div className="slide BrightnessBgm-icon" />
                <div
                  style={{ color: "#fff", height: "40px", lineHeight: "40px" }}
                >
              ...
              <div style={{ display: "flex", position: "relative" }}>
                ...
                <Slider
                  ...
                  value={
                    typeof this.state.brightVal === "number"
                      ? this.state.brightVal
                      : 0
                  }
                  step={5}
                />
                ...
                <Icon
                  type="plus"
                  onClick={this.addBrightness}
                  style={{ color: "#fff", marginTop: "12px" }}
                />
                &nbsp;&nbsp;
                <button
                  onClick={this.BrightnessDefault}
                  style={{ marginTop: "8px" }}
                  size="small"
                >
                  <Icon type="reload" />
            ...
            <div className="ant-slider-div">
              ...
              >
                <div className="slide Contrast-icon" />
                <div
                  style={{ color: "#fff", height: "40px", lineHeight: "40px" }}
                >
              ...
              <div style={{ display: "flex", position: "relative" }}>
                ...
                <Slider
                  ...
                  value={
                    typeof this.state.contrastVal === "number"
                      ? this.state.contrastVal
                      : 0
                  }
                  step={5}
                />
          ...
          <Col span={5}>
            <div style={{ height: "90px" }}>
              <img
                style={{ width: "214px", height: "76px", marginTop: "15px" }}
                src={this.state.histogramPath}
                alt=""
              />
            </div>
            ...
            <div className="ant-slider-div">
              ...
              <div style={{ display: "flex", position: "relative" }}>
                ...
                <Icon
                  type="plus"
                  style={{ color: "#fff", marginTop: "12px" }}
                  onClick={this.addGrayWindow}
                />
                &nbsp;&nbsp;
                <button size="small" style={{ marginTop: "8px" }}>
                  <Icon type="reload" onClick={this.revertGrayWindow} />
                </button>
              </div>
            </div>
          </Col>
            ...
            <Row style={{ marginTop: "5px" }}>
              <Col span={24} className="eightShow">
                <Button
                  ...
                  title="Reverse"
                  type={this.state.operationList[0].btnState}
                  onClick={this.handleBtnClick.bind(this, "Reverse")}
                />
                <Button
                  className="btn btn-Emboss"
                  title="Emboss"
                  type={this.state.operationList[1].btnState}
                  style={{ margin: "0 15px" }}
                  onClick={this.handleBtnClick.bind(this, "Emboss")}
                />
                <Button
                  className="btn btn-Sharpness"
                  title="Sharpness"
                  type={this.state.operationList[2].btnState}
                  onClick={this.handleBtnClick.bind(this, "Sharpness")}
                />
                <Button
                  className="btn btn-Smooth1"
                  title="Smooth1"
                  style={{ margin: "0 15px" }}
                  type={this.state.operationList[8].btnState}
                  onClick={this.handleBtnClick.bind(this, "Smooth1")}
                />
                <Button
                  className="btn btn-Smooth2"
                  title="Smooth2"
                  type={this.state.operationList[9].btnState}
                  onClick={this.handleBtnClick.bind(this, "Smooth2")}
                />
                <Button
                  className="btn btn-Smooth3"
                  title="Smooth3"
                  style={{ margin: "0 15px" }}
                  type={this.state.operationList[10].btnState}
                  onClick={this.handleBtnClick.bind(this, "Smooth3")}
                />
                <Button
                  className="btn btn-gray"
                  title="Gray"
                  type={this.state.operationList[13].btnState}
                  onClick={this.handleBtnAlgorithmServices.bind(this, "gray")}
                />
                ...
                <Button
                  ...
                  title="Mist"
                  style={{ margin: "0 15px" }}
                  type={this.state.operationList[14].btnState}
                  onClick={this.handleBtnAlgorithmServices.bind(this, "mist")}
                />
                <Button
                  className="btn btn-oasis"
                  title="Oasis"
                  type={this.state.operationList[15].btnState}
                  onClick={this.handleBtnAlgorithmServices.bind(this, "oasis")}
                />
                <Button
                  className="btn btn-settingsun"
                  title="Settingsun"
                  style={{ margin: "0 15px" }}
                  type={this.state.operationList[16].btnState}
                  onClick={this.handleBtnAlgorithmServices.bind(
                    this,
                    "settingsun"
                  )}
                />
            ...
            <Row style={{ marginTop: "20px" }}>
              <Col span={24} className="eightShow">
                <Button
                  ...
                  title="Grape"
                  type={this.state.operationList[17].btnState}
                  onClick={this.handleBtnAlgorithmServices.bind(this, "grape")}
                />
                <Button
                  className="btn btn-ocean"
                  title="Ocean"
                  style={{ margin: "0 15px" }}
                  type={this.state.operationList[18].btnState}
                  onClick={this.handleBtnAlgorithmServices.bind(this, "ocean")}
                />
                <Button
                  className="btn btn-pavonine"
                  title="Pavonine"
                  type={this.state.operationList[19].btnState}
                  onClick={this.handleBtnAlgorithmServices.bind(
                    this,
                    "pavonine"
                  )}
                ...
                <Button
                  className="btn btn-EdgeEnhance1"
                  title="EdgeEnhance1"
                  type={this.state.operationList[4].btnState}
                  style={{ margin: "0 15px" }}
                  onClick={this.handleBtnClick.bind(this, "EdgeEnhance1")}
                />
                <Button
                  className="btn btn-EdgeEnhance2"
                  title="EdgeEnhance2"
                  type={this.state.operationList[5].btnState}
                  onClick={this.handleBtnClick.bind(this, "EdgeEnhance2")}
                />
                <Button
                  className="btn btn-EdgeEnhance3"
                  title="EdgeEnhance3"
                  type={this.state.operationList[6].btnState}
                  style={{ margin: "0 15px" }}
                  onClick={this.handleBtnClick.bind(this, "EdgeEnhance3")}
                />
                <Button
                  className="btn btn-GrayImage"
                  title="GrayImage"
                  type={this.state.operationList[11].btnState}
                  onClick={this.handleBtnClick.bind(this, "GrayImage")}
                />
                <Button
                  className="btn btn-histogram"
                  title="Histogram"
                  loading={this.state.histogramLoading}
                  style={{ margin: "0 15px" }}
                  type={this.state.operationList[12].btnState}
                  onClick={this.handleBtnAlgorithmServices.bind(
                    this,
                    "histogram"
                  )}
                ...
                <Button
                  ...
                  title="Revert"
                  type={this.state.operationList[3].btnState}
                  onClick={this.handleBtnClick.bind(this, "Revert")}
                />

                <Button style={{ margin: "0 15px" }}  className="btn" title="Open" onClick={this.showDrawer}>
                ...
                >
                  <Form layout="vertical" hideRequiredMark>
                    ...
                    <Row gutter={16}>
                      ...
                      <div className="ant-drawer-header-child">
                          <div className="ant-drawer-title">Conclusion Information</div>
                      ...
                      <Col span={12}>
                        ...
                        >
                          <Select placeholder="Type">
                            <Option value="No Suspect" activeTexture>No Suspect</Option>
                            <Option value="With Suspect">With Suspect</Option>
                          </Select>
                        </Form.Item>
                      </Col>
                      <Col span={12}>
                    ...
                    <Row gutter={16}>
                      <Col span={12}>
                        <div
                            ...
                            >
                            <Button onClick={this.onClose} style={{ marginRight: 8 ,width:'200px'}}>
                                Cancel
                            </Button>
                      ...
                      <Col span={12}>
                        <div
                            ...
                            >
                            <Button onClick={this.submitInfo} type="primary" style={{ width:'200px'}}>
                                Submit
                            </Button>
                            </div>
                      </Col>
